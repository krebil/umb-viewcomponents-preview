...
internal class UnpublishedContent : IPublishedContent
{
    ...
    public UnpublishedContent(int id, ServiceContext serviceContext,
        IPublishedContentTypeFactory publishedContentTypeFactory, PropertyEditorCollection propertyEditors)
        ...
        : base()
    {
        ...
        this.contentType =
            new Lazy<IPublishedContentType>(() => _publishedContentTypeFactory.CreateContentType(contentType));
        this.creatorName = new Lazy<string>(() => this.content.GetCreatorProfile(userService.Value).Name);
        this.parent = new Lazy<IPublishedContent>(() =>
            new UnpublishedContent(serviceContext.ContentService.GetById(this.content.ParentId), serviceContext,
                _publishedContentTypeFactory, _propertyEditors));
        this.properties = new Lazy<Dictionary<string, IPublishedProperty>>(() => MapProperties(serviceContext));
        this.urlName = new Lazy<string>(() => this.content.Name.ToUrlSegment(new DefaultShortStringHelper(new DefaultShortStringHelperConfig())));
